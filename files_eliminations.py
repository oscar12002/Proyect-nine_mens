...
def activate(board_v, row, column):
    v_e =  [[[0,0],[0,3],[0,6]],
            [[1,1],[1,3],[1,5]],
            [[2,2],[2,3],[2,4]],
            [[3,0],[3,1],[3,2]],
    ...
    for convert in v_e:
        if [row, column] in convert:
            if board_v.board_ret[convert[0][0]][convert[0][1]] in ("◌","◍") and board_v.board_ret[convert[1][0]][convert[1][1]] in ("◌","◍") and board_v.board_ret[convert[2][0]][convert[2][1]] in ("◌","◍"):
                return True
        ...
        while eliminate_turn != 1:
            ...
            if self.file_eliminate.simbol == "○":
                ...
                if self.board_e.board_ret[row_eliminate][column_eliminate] == " ● ":
                    self.board_e.board_ret[row_eliminate][column_eliminate] = " · "
                    eliminate_turn += 1
            # (self.board_e.board_ret[convert[0][0]][convert[0][1]] == self.file_eliminate.m_simbol and self.board_e.board_ret[convert[1][0]][convert[1][1]] == " " + self.file_eliminate.m_simbol + " " and self.board_e.board_ret[convert[2][0]][convert[2][1]] == self.file_eliminate.m_simbol or
            # if the file selected is in a widmill the player can´t eliminate
            if activate(self.board_e, row_eliminate, column_eliminate) != True:  
                if self.file_eliminate.simbol == "●":
                    if self.board_e.board_ret[row_eliminate][column_eliminate] == "◌":
                        self.board_e.board_ret[row_eliminate][column_eliminate] = "·"
                        eliminate_turn += 1
